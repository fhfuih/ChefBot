            ...
            if (message.quick_replies) {
                ...
                for (var r = 0; r < message.quick_replies.length; r++) {
                    (function (reply) {
                        ...
                        el.href = '#';
                        if (!reply.image) {
                            // console.log(reply, 'with no image')
                            el.innerHTML = reply.title;
                        } else {
                            el.classList.add('image');
                            var cap = document.createElement('caption');
                            cap.innerHTML = reply.title;
                            // el.setAttribute('style', `background-image: url(${reply.image})`);
                            el.style = `background-image: url(${reply.image})`
                            el.appendChild(cap);
                        }

                        if (reply.disabled) {
                            el.setAttribute('disabled', '')
                            el.setAttribute('title', 'Not implemented')
                        } else {
                            el.onclick = function () {
